...
\begin{itemize}[leftmargin=*]
	...
	\item \textbf{Crawler:} BeautifulSoup4, httpx (HTTP client for page fetching).
...
\begin{itemize}[leftmargin=*]
	...
	\textbf{Implemented already:} three BeautifulSoup4 + httpx crawlers for real Romanian sources (bookzone.ro, carturesti.ro, libris.ro), normalization helpers (\texttt{normalize\_isbn}, \texttt{parse\_year}), deduplication pipeline in the ingest endpoint, ISBN/title/author cleanup, and a \texttt{run\_all.py} orchestrator with per-source summary stats.
	\textbf{Next:} strengthen deduplication rules further, improve crawl coverage and error recovery, and prepare review text for NLP.
...
\begin{itemize}[leftmargin=*]
	...
	Design and maintain BeautifulSoup4 + httpx crawlers, source adapters, data normalization, and ingestion quality checks; prepares Romanian review text datasets for NLP and supports integration tests for crawl-to-API flow.
